...
(defmacro compute-step (atrees i2 j2 k2 l2)
  (let ((atree-s (gensym))
        (atree-t (gensym)))
    `(when (<= 0 ,i2 ,j2 ,k2 ,l2 n)
       (dolist (,atree-s ,atrees)
         (dolist (,atree-t (matrix-b ,i2 ,j2 ,k2 ,l2))
    ...
    (do ((l 0 (+ l 1)))
      ...
      (do ((i l (- i 1)))
        ...
        (do ((j i (+ j 1)))
          ...
          (do ((k l (- k 1)))
              ((= k (1- j)))
            (when (pair-p j (1+ k))            ;; step(1-1)
              (compute-step t2u i (1- j) (1+ k) l))
            (when (pair-p (1+ i) l)            ;; step(1-2)
              (compute-step t2d (1+ i) j k (1- l)))
            (when (pair-p (1+ i) j)            ;; step(2-1)
              (compute-step t3l (1+ i) (1- j) k l))
            (when (pair-p (1+ k) l)            ;; step(2-2)
              (compute-step t3r i j (1+ k) (1- l)))
            (compute-step t4ld (1+ i) j k l)   ;; step(3-1)
            (compute-step t4lu i (1- j) k l)   ;; step(3-2)
            (compute-step t4rd i j k (1- l))   ;; step(3-3)
            (compute-step t4ru i j (1+ k) l)   ;; step(3-4)
            )
          )
        )
      )
