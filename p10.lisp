...
(defun make-tag2rna-general-parser ()
  (make-tag2rna-parser
   '(s) '(s)
   '((t2d s s) (t2u s s) (t3l s s) (t3r s s)
     (t4ld s s) (t4lu s s) (t4rd s s) (t4ru s s))))
...
(defmacro atree-next-atree (atree)
  `(fourth ,atree))
    ...
    `(dolist (,atree-s ,atrees)
       (dolist (,atree-t ,element-of-b)
         (when (eq (atree-tagged-node ,atree-s)
           ...
           (pushnew
            (reverse (cons ,atree-t (reverse ,atree-s)))
            (matrix-b i j k l))
           (return))))))
